...
describe("Create/Update Comments", function () {
  ...
  it("Should get three users", async () => {
    const res = await fetch(`${URL}/api/users`).then(r => r.json());
  ...
  it("Should find the user Donald Duck", async () => {
    const res = await fetch(`${URL}/api/users/dd@b.dk`).then(r => r.json());
    expect(res.userName).to.be.equal("dd@b.dk");
  })

  it("Should not find the user xxx@b.dk", async () => {
    const res = await fetch(`${URL}/api/users/dd@b.dk`).then(r => r.json()).catch(err => {
  ...
  it("Should Remove the user Donald Duck", async () => {
    ...
    const res = await fetch(`${URL}/api/users/dd@b.dk`, config).then(r => r.json());
    expect(res.status).to.be.equal("User was deleted");
  })
